...
class WorkerController {
	...
	static async getWorkers(req, res) {
		try {
			...
			return res
				.status(500)
				.json({ message: "Something when wrong!!", error: err });
		}
	}
		...
		try {
			...
			koneksi.query(queryInsert, data, (err, rows, field) => {
				if (err) {
					return res
						.status(500)
						.json({ message: "Failed Insert Data!!", error: err });
				}
					...
					koneksi.query(
						...
						(err, rows, field) => {
							if (err) {
								return res
									.status(500)
									.json({ message: "Failed Update Data", error: err });
							}

							res
								.status(200)
								.json({ success: true, message: "Success Update Data" });
						}
					);
				} else {
					return res
				...
				if (rows.length) {
					koneksi.query(queryDelete, req.params.id, (err, rows, field) => {
						if (err) {
							return res
								.status(500)
								.json({ message: "Failed Delete Data", error: err });
						}

						res
							.status(200)
							.json({ success: true, message: "Success Delete Data" });
					});
				} else {
					return res
						.status(404)
